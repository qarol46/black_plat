...
AllanVarianceComputor::AllanVarianceComputor(ros::NodeHandle& nh, std::string config_file, std::string output_path)
  ...
  ROS_INFO_STREAM("Output file will be: " << imu_output_file_);
}
...
void AllanVarianceComputor::run(std::string bag_path) {
  ...
  if (!av_output_.is_open()) {
    ROS_ERROR_STREAM("FATAL: Cannot open output file: " << imu_output_file_);
    ROS_ERROR_STREAM("Check directory permissions: " << boost::filesystem::path(imu_output_file_).parent_path());
    return;
  }
  ROS_INFO_STREAM("Writing output to: " << imu_output_file_);
