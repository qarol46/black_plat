exact command to run docker:

```
docker run   -it   --rm   --net=host   --ipc=host   --pid=host   --gpus all --runtime=nvidia   -e=$DISPLAY   -e=ROS_DOMAIN_ID  -v /dev/dri:/dev/dri -v /tmp/.X11-unix:/tmp/.X11-unix -v ~/Projects/GLIM/src/glim/config:/glim/config   koide3/glim_ros2:humble   ros2 run glim_ros glim_rosnode --ros-args -p config_path:=/glim/config
```
