...
public:
    ...
    TransformFusion(const rclcpp::NodeOptions & options) : ParamServer("liwo_sam_transformFusion", options)
    {
        ...
        pubImuPath = create_publisher<nav_msgs::msg::Path>("liwo_sam/imu/path", qos);
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        //{
            ...
            //RCLCPP_INFO(get_logger(), "TF published successfully");
        //}else{
            RCLCPP_INFO(get_logger(), "TF published false");
        //}
...
public:
    ...
    IMUPreintegration(const rclcpp::NodeOptions & options) :
            ParamServer("liwo_sam_imu_preintegration", options)
    {
    ...
    void odometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        while (!imuQueOpt.empty())
        {
            ...
            if (imuTime < currentCorrectionTime - delta_t)
            {
                ...
                // Для diff_drive_robot используем только ускорение по X, остальные обнуляем
                gtsam::Vector3 acc;
                if (diffDriveRobot)
                {
                    // В режиме diff_drive считаем, что робот может двигаться только вперед/назад
                    // Ускорения по Y и Z игнорируем (они должны быть отфильтрованы или обнулены)
                    acc = gtsam::Vector3(
                        thisImu->linear_acceleration.x,  // Только X ускорение
                        0.0,
        ...
        if (!imuQueImu.empty())
        {
            ...
            for (int i = 0; i < (int)imuQueImu.size(); ++i)
            {
                ...
                // Для diff_drive_robot используем только ускорение по X
                gtsam::Vector3 acc;
                if (diffDriveRobot)
                {
                    acc = gtsam::Vector3(
